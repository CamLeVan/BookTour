            ...
            <div class="col-md-8">
                <div class="success-card">
                    ...
                    <div class="booking-details">
                        ...
                        @php
                            $origPrice = (float) $booking->total_price;
                            $discAmt = (float) $booking->discount_amount;
                            $netPrice = max(0, $origPrice - $discAmt);
                            $isDep = $booking->is_deposit;
                            $paidNow = $isDep ? $booking->deposit_amount : ($booking->total_amount ?? $netPrice);
                            $remAmt = $booking->remaining_amount;
                        @endphp
                        ...
                        <div class="detail-item">
                            ...
                            <span class="value">
                                @if($isDep)
                                    <span class="badge bg-warning text-dark p-2">💰 Đặt cọc 30% giữ chỗ</span>
                                @else
                        ...
                        @if($discAmt > 0)
                            <div class="detail-item">
                                <span class="label">Giá tour gốc:</span>
                                <span class="value">{{ number_format($origPrice) }} VNĐ</span>
                            </div>
                            <div class="detail-item text-success">
                                <span class="label text-success">Mã giảm giá ({{ $booking->voucher->code ?? '' }}):</span>
                                <span class="value">-{{ number_format($discAmt) }} VNĐ</span>
                            </div>
                        @endif

                        <div class="detail-item">
                            <span class="label">Tổng giá trị tour ròng:</span>
                            <span class="value fw-bold text-dark">{{ number_format($netPrice) }} VNĐ</span>
                        </div>

                        @if($isDep)
                            <div class="detail-item bg-light p-2 rounded">
                                <span class="label text-dark fw-bold">💰 Số tiền đã trả (30% cọc):</span>
                                <span class="value text-warning fw-bold fs-5">{{ number_format($paidNow) }} VNĐ</span>
                            </div>
                            <div class="detail-item">
                                <span class="label text-danger">Số tiền nợ còn lại (70%):</span>
                                <span class="value text-danger fw-bold">{{ number_format($remAmt) }} VNĐ</span>
                            </div>
                        @endif

                        <div class="detail-item total">
                            <span class="label">Tổng tiền đã thanh toán ngay:</span>
                            <span class="value text-success fs-4 fw-bold">{{ number_format($paidNow) }} VNĐ</span>
                        </div>
                    </div>
