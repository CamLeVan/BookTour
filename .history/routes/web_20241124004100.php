...
Route::middleware(['auth'])->group(function () {
    Route::prefix('bookings')->name('frontend.bookings.')->group(function () {
        ...
        Route::post('/tours/{tour}/book', [BookingController::class, 'store'])->name('store');
        
        // Review và thanh toán
        Route::get('/{pendingBooking}/review', [BookingController::class, 'review'])->name('review');
        Route::post('/{pendingBooking}/confirm', [BookingController::class, 'confirm'])->name('confirm');
        
        // Kiểm tra trạng thái và hoàn thành
        Route::get('/{pendingBooking}/status', [PaymentController::class, 'checkStatus'])->name('check-status');
        Route::get('/{pendingBooking}/success', [BookingController::class, 'success'])->name('success');
        
        // Route giả lập thanh toán (chỉ cho môi trường development)
        if (config('app.env') === 'local') {
            Route::post('/{pendingBooking}/simulate-payment', [PaymentController::class, 'simulatePayment'])
