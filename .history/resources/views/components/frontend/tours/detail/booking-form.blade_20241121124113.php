<div class="right-sidebar">
    <div class="right-sidebar-item booking-form">
        ...
        <div class="price-section mb-4">
            ...
            @else
                <h3 class="regular-price">{{ number_format($tour->price) }}đ</h3>
            @endif
        </div>

        <form method="POST" 
              action="{{ route('frontend.tours.booking') }}" 
              class="booking-form">
            @csrf
            ...
            <div class="form-group mb-4">
                ...
                <textarea name="notes" 
                          class="form-control"
                          rows="3"
                          placeholder="Yêu cầu đặc biệt...">{{ old('notes') }}</textarea>
            </div>

            <!-- Hiển thị tổng tiền -->
            <div class="total-section mb-4">
                <div class="d-flex justify-content-between mb-2">
                    <span>Người lớn (<span id="adults-count">1</span> x {{ number_format($tour->price) }}đ)</span>
                    <span id="adults-total">{{ number_format($tour->price) }}đ</span>
                </div>
                <div class="d-flex justify-content-between mb-2">
                    <span>Trẻ em (<span id="children-count">0</span> x {{ number_format($tour->price * 0.5) }}đ)</span>
                    <span id="children-total">0đ</span>
                </div>
                <div class="d-flex justify-content-between fw-bold">
                    <span>Tổng cộng</span>
                    <span id="total-price" 
                          data-price="{{ $tour->sale_price ?? $tour->price }}">
                        {{ number_format($tour->price) }}đ
                    </span>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateTotal() {
        ...
        $('#adults-total').text(formatMoney(adultsTotal) + 'đ');
        $('#children-total').text(formatMoney(childrenTotal) + 'đ');
        $('#total-price').text(formatMoney(total) + 'đ');
    }
