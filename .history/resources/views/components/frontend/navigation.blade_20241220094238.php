...
@push('styles')
<style>
    /* Modern Travel-themed Modal Styles */
    ...
    .auth-modal .form-control:focus {
        border-color: #FF6B6B;
        box-shadow: 0 8px 16px -4px rgba(255, 107, 107, 0.15);
    ...
    .auth-modal .btn-forgot {
        background: linear-gradient(135deg, #FFB88C 0%, #ffa677 100%);
        color: white;
        border: none;
        box-shadow: 0 10px 20px -5px rgba(255, 184, 140, 0.3);
    }
    ...
    .auth-modal .social-btn:hover {
        ...
        transform: translateY(-2px);
        border-color: #FF6B6B;
        box-shadow: 0 10px 15px -3px rgba(255, 107, 107, 0.1);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hiển thị modal reset password n��u có token
    ...
    $('#loginForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Đăng nhập thành công - reload trang hoặc redirect
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    // Rate limiting
                    errorDiv.html('Quá nhiều lần thử. Vui lòng thử lại sau.').show();
                } else {
                    // Other errors
                    errorDiv.html('Có lỗi xảy ra. Vui lòng thử lại.').show();
                }
            }
        });
    });
