...
$(document).ready(function() {
    ...
    $('#updateProfileForm').on('submit', function(e) {
        e.preventDefault();
        handleFormSubmit($(this), $(this).attr('action'), function(response) {
            // Tải lại trang nếu cập nhật thành công
    ...
    $('#registerForm').on('submit', function(e) {
        ...
        const form = $(this);
        const actionUrl = form.attr('action'); // URL gửi yêu cầu
        ...
        $.ajax({
            ...
            data: form.serialize(),
            success: function(response) {
                // Nếu đăng ký thành công, chuyển hướng đến trang home
                window.location.href = '/'; // Hoặc route bạn muốn
            },
            error: function(xhr) {
                // Nếu có lỗi, hiển thị lỗi trên modal
                const errors = xhr.responseJSON.errors;
                for (let field in errors) {
                    const inputField = form.find(`[name="${field}"]`);
                    inputField.addClass('is-invalid'); // Thêm class lỗi
                    inputField.next('.invalid-feedback').remove(); // Xóa thông báo lỗi cũ
                    inputField.after(`<div class="invalid-feedback">${errors[field][0]}</div>`); // Thêm thông báo lỗi mới
                }
            }
        });
    });
