        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    errorDiv.html(errorMessages).show();
...
$(document).ready(function() {
    $('#registerForm').on('submit', function(e) {
        ...
        // Gửi yêu cầu AJAX
        $.ajax({
            url: $(this).attr('action'), // Đường dẫn đến route đăng ký
            method: 'POST',
            data: $(this).serialize(),
            success: function(response) {
                // Nếu đăng ký thành công, bạn có thể chuyển hướng hoặc hiển thị thông báo thành công
                toastr.success('Đăng ký thành công!');
                $('#registerModal').modal('hide'); // Đóng modal nếu cần
            },
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    $('#errorMessages').html(errorMessages).show(); // Hiển thị tất cả lỗi
                } else {
                    $('#errorMessages').text('Có lỗi xảy ra, vui lòng thử lại!').show();
                }
            }
