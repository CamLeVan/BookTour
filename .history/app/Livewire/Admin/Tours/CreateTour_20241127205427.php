...
class CreateTour extends Component
{
    ...
    // 3 Ảnh phụ
    #[Validate(['images.*' => 'image|max:1024', message: [
        'image' => 'File không đúng định dạng',
        'max' => 'Mỗi ảnh không được vượt quá 1MB'
    ]])]
    public $images = [];
    public $images_arr = [];
    ...
    public function createTour()
    {
        ...
        try {
            ...
            $mainImagePath = 'public/tours/' . $mainImageName;
            
            // Resize và lưu ảnh chính
            $img = Image::make($this->image->getRealPath());
            $img->fit(800, 600); // Kích thước cố định cho ảnh chính
            $img->save(storage_path('app/' . $mainImagePath));

            // Xử lý 3 ảnh phụ
            $subImagePaths = [];
            foreach ($this->images as $index => $image) {
                $subImageName = 'sub_' . time() . '_' . ($index + 1) . '_' . $image->getClientOriginalName();
                $subImagePath = 'public/tours/' . $subImageName;
                
                // Resize và lưu ảnh phụ
                $img = Image::make($image->getRealPath());
                $img->fit(400, 300); // Kích thước nhỏ hơn cho ảnh phụ
                $img->save(storage_path('app/' . $subImagePath));
